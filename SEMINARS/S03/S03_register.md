...
| **NFR-007** | US-001 – Регистрация пользователя | **RateLimiting**                 | Лимит: 10 запросов в мин с IP; превышение → 429 + Retry-After                                                                    | Защита от спама и brute-force                              | **Given** активный IP <br />**When** > 10 POST `/api/register` за 60 с <br />**Then** 429 и корректный заголовок Retry-After                                                                                                   | e2e:`register-rate-limit`; logs 429                                  |                    | backend-team | Draft  | P2 – High   | S2 – Major | security,rate       |
...
| **NFR-009** | US-001 – Регистрация пользователя | **Observability/Logging**        | Все запросы логируются в JSON с X-Correlation-ID                                                                                        | Трассировка и диагностика                         | **Given** запрос с X-Correlation-ID <br />**When** он проходит через сервис <br />**Then** все логи содержат correlation_id и ключевые поля                                                             | log pattern sample; query screenshot                                   |                    | backend-team | Draft  | P2 – Medium | S2 - Major  | logs,observability  |
| **NFR-010** | US-001 – Регистрация пользователя | **Data-Integrity**               | Все SQL-запросы параметризованы; значения нормализованы (NFC/E.164/UTC)                                         | Целостность и устойчивость к инъекциям | **Given** входные данные в нестандартном формате <br />**When** выполняется запись в БД <br />**Then** используется параметризация; значения нормализованы | пример ORM-кода; DB schema policy                            |                    | backend-team | Draft  | P2 – Medium | S2 – Major | data,integrity      |
